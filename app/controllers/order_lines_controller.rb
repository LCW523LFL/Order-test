class OrderLinesController < ApplicationController
  ...
  def join
    # @order_lines = OrderLine.joins(:products).select("order_lines.*,products.*").find(params[:id])
    @order_lines= OrderLine.where("order_lines.id=?",params[:id]).joins(:products).select("products.order_line_id,order_lines.num,products.name,products.detail,order_lines.time")
